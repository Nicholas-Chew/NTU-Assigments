...
//////////////////////////////////////////////////////////////////////////////////
 ...
 void createQueueFromLinkedList(LinkedList *ll, Queue * q)
{
	//declare node to travese through ll
	ListNode *cur = ll->head;

	//Loop if there is still node to manipulate
	while(cur != NULL)
	{
		//Enqueue / insert item to the queue
		enqueue(q,cur->item);
		//Go to the next node 
		cur = cur->next;
	}
...
void removeEvenValues(Queue *q)
{
	//value: temporary store the dequeued item
	//i: loop variable 
	//qSize: the initial size of the queue
	int value, i = 0, qSize;
	qSize = q->ll.size;

	for(i = 0 ; i<qSize; i++)
	{
		//Dequeue each values from the queue and store in temporary value
		value = dequeue(q);

		//Check is the value odd, enqueue the value back if it is a odd number
		if(value%2==1)
			enqueue(q,value);
	}
